...
## ٣-ب. اعتماد البريد (لإرسال التقارير)

بعد اجتياز بوابة الأرقام يُرسل التقرير بريدًا. الناقل الافتراضي **Gmail API**:

1. **Credentials → New → Gmail OAuth2 API**، سمّه `Gmail account`، واربطه بالحساب
   المُرسِل. تأكد أن شاشة موافقة جوجل تطلب صلاحية **Send email on your behalf**
   (`gmail.send`) — الاعتماد بصلاحية قراءة فقط يفشل بخطأ غامض.
2. افتح عقدة **`📧 Gmail: Send Report`** واختر الاعتماد.
3. في الشيت، املأ عمود **«مستلمو التقرير»**. ضع عنوان Gmail بينها في أول تشغيلة
   للتأكد من وصول الرسائل خارج دومينك.

لمسار SMTP بدلًا من ذلك: اربط اعتماد `SMTP account` بعقدة `📧 SMTP: Send Report`،
واضبط في الملف التعريفي `delivery.email.transport: 'smtp'` مع `from_email`.

> **إن وصلت الرسائل إلى دومينك ولم تصل إلى Gmail**، فالعلة في الناقل لا في
> الكود — الأسباب الأربعة وحلولها في [`EMAIL.md`](EMAIL.md).
...
| عمود «بوابة الأرقام» يقول `🔴 محجوز` | رقم في النص لا يطابق مصدره أو بلا مصدر | **ليس عطلًا — هذا عمل البوابة.** افتح «ملاحظات الأرقام» ونفّذ [جدول الأعطال](REVIEW-PLAN.md#ج-جدول-الأعطال--ماذا-يعني-كل-بلاغ-وماذا-تفعل-به) |
| التقرير لم يُرسل والحالة محجوزة | البوابة منعت تسليم أرقام مشكوك فيها | أصلح الأرقام وأعد التشغيل؛ العميل تلقّى إشعار حجز يشرح السبب |
| الرسائل تصل للدومين لا لـ Gmail | خدمة SMTP relay مقيّدة بدومينك، أو SPF/DKIM/DMARC ناقصة | [`EMAIL.md`](EMAIL.md) — وافحص مجلد Spam أولًا |
| «لا يوجد أي مستلم صالح» في حالة الإرسال | عمود «مستلمو التقرير» فارغ و`delivery.email.to` فارغ | املأ أحدهما |
| عنوان صحيح لم يصله التقرير | استُبعد لسبب مسجَّل | اقرأ عمود «حالة الإرسال» — كل استبعاد مكتوب بسببه |
| بلاغات `first_party_number` كثيرة ومتكررة | أرقام العميل عن نفسه | بعد تأكيد العميل كتابةً، أضِفها إلى `numbers.allow_values` في الملف التعريفي |
