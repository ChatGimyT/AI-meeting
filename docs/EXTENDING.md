...
## ضبط سياسة الأرقام لعميل جديد

كل شدّة مراجعة الأرقام في مفتاح واحد داخل الملف التعريفي:

<div dir="ltr">

```js
numbers: {
  severity: { stat: 'high', bare: 'medium', first_party: 'medium' },
  allow_rounding: true,     // 27.4% في المصدر → 27% في النص
  proximity_chars: 700,     // كم يبعد رابط المصدر عن الرقم
  allow_values: [2014, 12], // أرقام يملكها العميل بلا مصدر خارجي
...
| الحالة | الضبط |
|---|---|
| **محتوى طبي أو مالي أو قانوني** | `bare: 'high'` و`allow_rounding: false` — النقل الحرفي وحده |
| **مقال رأي أو محتوى علامة بلا إحصائيات** | `stat: 'medium'` — يُبلَّغ ولا يحجز |
| **العميل يعيد نفس أرقامه كل مرة** | أضِفها إلى `allow_values` بعد تأكيده الكتابي |
| **ترغب في تقرير بلا حجز إطلاقًا** | `block_delivery_on: []` — تُبلَّغ المخالفات ويُرسل التقرير (لا يُنصح به) |

**لإضافة فحص رقمي جديد** عدّل `N.auditArticle` في `src/lib/numbers.js` واستدعِ
`flag(code, severity, tok, message, extra)`. **ولإضافة رقم جديد إلى مصالحة
التقرير** أضِف سطرًا إلى مصفوفة `H.N.recon` في
`src/stages/27-numbers-reconcile.js` بمصدره الذي يُعاد اشتقاقه منه.

> **اكتب الاختبار في `tools/numbers-test.mjs` قبل الفحص لا بعده.** فحص بلا
> اختبار يمرّ صامتًا يوم ينكسر، وهو أخطر من غيابه.
> المنهجية كاملة: [`NUMBERS-AUDIT.md`](NUMBERS-AUDIT.md).
