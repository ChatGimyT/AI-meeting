...
//   رقم          = ترتيب مؤكد
// مفتاح العمود بيختلف حسب الإيقاع: شهري '2026-03' وأسبوعي '2026-W12'.
// بيتقرا من Config عشان النود يفضل واحد للتقريرين.
const __cadence = $('Config').first().json.cadence;
const COL_RE = __cadence === 'weekly'
  ? /^(\d{4}-W\d{2})\s+(Pos|Impr)$/
  : /^(\d{4}-\d{2})\s+(Pos|Impr)$/;
